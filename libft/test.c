...
static size_t	ft_endsofsubs(char const *s, char c, size_t start)
{
...
static char	**ft_bufferfree(char **buffer)
{
	size_t	i;
...
char	**ft_split(char const *s, char c)
{
	...
	if (s == NULL)
		return (NULL);
	buffer = malloc((ft_wordcounter(s, c) + 1) * sizeof(char *));
	if (buffer == NULL)
		return (NULL);
	while (s[start] != '\0')
	{
		...
		if (s[start] == '\0')
			break ;
		buffer[i] = (ft_substr(s, start, ft_endsofsubs(s, c, start)));
		if (buffer[i] == NULL)
			return (ft_bufferfree(buffer));
		i++;
		start += ft_endsofsubs(s, c, start);
	}
	buffer[i] = NULL;
	return (buffer);
}
